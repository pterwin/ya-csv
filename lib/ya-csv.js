...
CsvWriter.prototype.writeRecord = function(rec, callback) {
    if (!rec) return; // ignore empty records
    if (!Array.isArray(rec)) {
        throw new Error("CsvWriter.writeRecord only takes an array as an argument");
    }
    return _writeArray(this, rec, callback);
};

function _writeArray(writer, arr, callback) {
    var out = [];
    ...
    out.push("\r\n");
    return writer.writeStream.write(out.join(''), this.encoding, callback);
};
